...
df.s_2019 <- st_drop_geometry(HabSel) 
df.s_2019 <- subset(df.s_2019, df.s_2019$year==2019)

# Ensure factors are correctly set
df.s_2019$LC_cat <- as.factor(df.s_2019$LC_cat)
df.s_2019$Season <- as.factor(df.s_2019$Season) 
df.s_2019$ID <- as.factor(df.s_2019$ID)

# Subset presence points
df.s_2019 <- subset(df.s_2019, df.s_2019$Pres == 1)

# Calculate proportional use of each habitat category by season for each individual
prop_use_2019 <- df.s_2019 %>%
...
prop_use_wide_2019 <- prop_use_2019 %>%
  pivot_wider(names_from = Season, values_from = prop, values_fill = list(prop = 0))

# Function to calculate Schoener's D
schoeners_d_2019 <- function(df.s_2019) {
  1 - 0.5 * sum(abs(df.s_2019$Breeding - df.s_2019$Wintering))
}  

# Apply the function to each individual
schoeners_d_values_2019 <- prop_use_wide_2019 %>%
  group_by(ID) %>% 
  summarise(Schoeners_D = schoeners_d_2019(cur_data()), .groups = 'drop') #this uses the dplyr package

# View Schoener's D values
print(schoeners_d_values_2019)

# Set seed for reproducibility
set.seed(123)

# Function to perform a permutation test
perm_test_2019 <- function(df.s_2019, n_permutations = 1000) {
  observed_d <- schoeners_d(df.s_2019)
...
perm_test_results_2019 <- prop_use_wide_2019 %>%
  group_by(ID) %>%
  summarise(test_result = list(perm_test(cur_data())), .groups = 'drop')

# Extract p-values and null distributions
p_values_2019 <- perm_test_results_2019 %>%
  transmute(ID, p_value = map_dbl(test_result, "p_value"))

null_distributions_2019 <- perm_test_results_2019 %>%
  transmute(ID, null_d = map(test_result, "null_d"))

# Combine results with Schoener's D values
results_2019 <- schoeners_d_values_2019 %>%
  inner_join(p_values_2019, by = "ID")

print(results_2019)

# Interpretation of Each Individual

# Eric: Schoener's D = 0.518, p-value = 0.106
# Moderate overlap, not significantly different from random expectations. No niche-tracking.

# Jose: Schoener's D = 0.592, p-value = 0.069
# Moderate overlap, not significantly different from random expectations. No niche-tracking.

# Ollie:  Schoener's D = 0.287, p-value = 0.237
# Low overlap, not significantly different from random expectations. No niche-tracking.

# Rufus:  Schoener's D = 0.302, p-value = 0.546
# Low overlap, not significantly different from random expectations. No niche-tracking.

# Summary
# Significant niche-tracking (p-value < 0.05): none
# Possible niche-tracking (borderline significant, 0.05 < p-value < 0.1): Jose
# No significant niche-tracking: Eric, Ollie, Rufus

# SOME PLOTS


# Combine observed and null distributions into a single dataframe
null_distribution_df_2019 <- map2_dfr(null_distributions_2019$null_d, null_distributions_2019$ID, function(nd, id) {
  data.frame(Schoeners_D = nd, Type = "Null", ID = id)
}) %>%
  bind_rows(data.frame(Schoeners_D = results_2019$Schoeners_D, Type = "Observed", ID = results_2019$ID))

# Boxplot of Schoener's D Values vs. Null Distribution
ggplot(null_distribution_df_2019, aes(x = ID, y = Schoeners_D)) +
  geom_boxplot() +
...
# Same but horizontal
ggplot(null_distribution_df_2019, aes(y = ID, x = Schoeners_D)) +
  geom_boxplot() +
...
ggplot(null_distribution_df_2019, aes(x = Schoeners_D, y = ID)) +
